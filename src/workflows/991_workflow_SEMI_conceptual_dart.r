...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    boosting = "dart", # puede ir  dart,gbdt  , ni pruebe random_forest
    objective = "binary",
    metric = "custom",
    first_metric_only = TRUE,
    boost_from_average = TRUE,
...
wf_SEMI_sep <- function( pnombrewf )
{
  ...
  CA_catastrophe_base( metodo="MICE") #"MachineLearning"
  FEintra_manual_base()
  DR_drifting_base(metodo="deflacion") #rank_cero_fijo
  FEhist_base()
